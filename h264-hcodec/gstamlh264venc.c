...
static gboolean
gst_amlh264venc_add_v_chroma_format (GstAmlH264VEnc *encoder, GstStructure * s)
{
...
static void gst_amlh264venc_finalize (GObject * object);
static gboolean gst_amlh264venc_start (GstVideoEncoder * encoder);
static gboolean gst_amlh264venc_stop (GstVideoEncoder * encoder);
static gboolean gst_amlh264venc_flush (GstVideoEncoder * encoder);

static gboolean gst_amlh264venc_init_encoder (GstAmlH264VEnc * encoder);
static gboolean gst_amlh264venc_set_roi(GstAmlH264VEnc * encoder);
static void gst_amlh264venc_fill_roi_buffer(guchar* buffer, gint buffer_w, gint buffer_h,
    struct RoiParamInfo *param_info, gint vframe_w, gint vframe_h, gint block_size);
static void gst_amlh264venc_close_encoder (GstAmlH264VEnc * encoder);

static GstFlowReturn gst_amlh264venc_finish (GstVideoEncoder * encoder);
static GstFlowReturn gst_amlh264venc_handle_frame (GstVideoEncoder * encoder,
    GstVideoCodecFrame * frame);
static GstFlowReturn gst_amlh264venc_encode_frame (GstAmlH264VEnc * encoder,
    GstVideoCodecFrame * frame);
static gboolean gst_amlh264venc_set_format (GstVideoEncoder * video_enc,
    GstVideoCodecState * state);

static gboolean gst_amlh264venc_propose_allocation (GstVideoEncoder * encoder,
    GstQuery * query);

static void gst_amlh264venc_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_amlh264venc_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

#define gst_amlh264venc_parent_class parent_class
G_DEFINE_TYPE_WITH_CODE (GstAmlH264VEnc, gst_amlh264venc, GST_TYPE_VIDEO_ENCODER,
...
static GstCaps *
gst_amlh264venc_sink_getcaps (GstVideoEncoder * enc, GstCaps * filter)
{
  ...
  for (i = 0; i < gst_caps_get_size (supported_incaps); i++) {
    ...
    for (j = 0; j < gst_caps_get_size (allowed); j++) {
      ...
      GstAmlH264VEnc *venc = GST_AMLH264VENC (enc);
      ...
      gst_amlh264venc_add_v_chroma_format (venc, s);
...
static gboolean
gst_amlh264venc_sink_query (GstVideoEncoder * enc, GstQuery * query)
{
...
static void cleanup_roi_param_list (GstAmlH264VEnc *encoder) {
...
static struct RoiParamInfo *retrieve_roi_param_info(GstAmlH264VEnc *encoder, gint id) {
...
static void
gst_amlh264venc_class_init (GstAmlH264VEncClass * klass)
{
  ...
  gobject_class->set_property = gst_amlh264venc_set_property;
  gobject_class->get_property = gst_amlh264venc_get_property;
  gobject_class->finalize = gst_amlh264venc_finalize;

  gstencoder_class->set_format = GST_DEBUG_FUNCPTR (gst_amlh264venc_set_format);
  gstencoder_class->handle_frame =
      GST_DEBUG_FUNCPTR (gst_amlh264venc_handle_frame);
  gstencoder_class->start = GST_DEBUG_FUNCPTR (gst_amlh264venc_start);
  gstencoder_class->stop = GST_DEBUG_FUNCPTR (gst_amlh264venc_stop);
  gstencoder_class->flush = GST_DEBUG_FUNCPTR (gst_amlh264venc_flush);
  gstencoder_class->finish = GST_DEBUG_FUNCPTR (gst_amlh264venc_finish);
  gstencoder_class->getcaps = GST_DEBUG_FUNCPTR (gst_amlh264venc_sink_getcaps);
  gstencoder_class->propose_allocation =
      GST_DEBUG_FUNCPTR (gst_amlh264venc_propose_allocation);
  gstencoder_class->sink_query = GST_DEBUG_FUNCPTR (gst_amlh264venc_sink_query);
  ...
  gst_element_class_set_static_metadata (element_class,
    "Amlogic h264 Encoder",
    "Codec/Encoder/Video",
    "Amlogic h264 Encoder Plugin",
  ...
  gst_amlh264venc_add_v_chroma_format (NULL, gst_caps_get_structure (supported_sinkcaps, 0));
 ...
 */
static void
gst_amlh264venc_init (GstAmlH264VEnc * encoder)
{
...
static void
gst_amlh264venc_finalize (GObject * object)
{
  G_OBJECT_CLASS (parent_class)->finalize (object);
}

static gboolean
gst_amlh264venc_start (GstVideoEncoder * encoder)
{
  GstAmlH264VEnc *venc = GST_AMLH264VENC (encoder);
...
static gboolean
gst_amlh264venc_stop (GstVideoEncoder * encoder)
{
  GstAmlH264VEnc *venc = GST_AMLH264VENC (encoder);

  gst_amlh264venc_close_encoder (venc);
...
static gboolean
gst_amlh264venc_flush (GstVideoEncoder * encoder)
{
  GstAmlH264VEnc *venc = GST_AMLH264VENC (encoder);

  gst_amlh264venc_init_encoder (venc);

  return TRUE;
}

/*
 * gst_amlh264venc_init_encoder
 ...
 */
static gboolean
gst_amlh264venc_init_encoder (GstAmlH264VEnc * encoder)
{
  GstVideoInfo *info;
  ...
  gst_amlh264venc_close_encoder (encoder);
  ...
  encoder->codec.handle = vl_video_encoder_init(encoder->codec.id, encode_info.width,encode_info.height,encode_info.frame_rate,encode_info.bit_rate,encode_info.gop,encode_info.img_format, qp_tbl.qp_I_min, qp_tbl.qp_I_max,qp_tbl.qp_P_min,qp_tbl.qp_P_max);
  ...
  if (!gst_amlh264venc_set_roi (encoder)) {
    return FALSE;
  }
...
/*
 * gst_amlh264venc_set_roi
 * @encoder:  update encoder roi value.
 *
 * Set roi value
 */
static gboolean
gst_amlh264venc_set_roi(GstAmlH264VEnc * encoder)
{
  GstVideoInfo *info;
  ...
  if (encoder->roi.enabled) {
    ...
    list_for_each(pos, &encoder->roi.param_info) {
      ...
      gst_amlh264venc_fill_roi_buffer(
...
static void
gst_amlh264venc_fill_roi_buffer(guchar* buffer, gint buffer_w, gint buffer_h,
    struct RoiParamInfo *param_info, gint vframe_w, gint vframe_h, gint block_size) {
...
static gboolean
idle_set_roi(GstAmlH264VEnc * self) {
  if (self != NULL) {
    gst_amlh264venc_set_roi (self);
  }
...
static gboolean
gst_amlh264venc_set_profile_and_level (GstAmlH264VEnc * encoder, GstCaps * caps)
{
  GstStructure *s;
  const gchar *profile;
...
/* gst_amlh264venc_set_src_caps
 * Returns: TRUE on success.
 */
static gboolean
gst_amlh264venc_set_src_caps (GstAmlH264VEnc * encoder, GstCaps * caps)
{
  GstCaps *outcaps;
  ...
  if (!gst_amlh264venc_set_profile_and_level (encoder, outcaps)) {
...
static void
gst_amlh264venc_set_latency (GstAmlH264VEnc * encoder)
{
...
static gboolean
gst_amlh264venc_set_format (GstVideoEncoder * video_enc,
    GstVideoCodecState * state)
{
  GstAmlH264VEnc *encoder = GST_AMLH264VENC (video_enc);
  ...
  if (!gst_amlh264venc_init_encoder (encoder))
    return FALSE;

  if (!gst_amlh264venc_set_src_caps (encoder, state->caps)) {
    gst_amlh264venc_close_encoder (encoder);
    return FALSE;
  }

  gst_amlh264venc_set_latency (encoder);

  return TRUE;
}

static GstFlowReturn
gst_amlh264venc_finish (GstVideoEncoder * encoder)
{
  return GST_FLOW_OK;
}

static gboolean
gst_amlh264venc_propose_allocation (GstVideoEncoder * encoder, GstQuery * query)
{
  GstAmlH264VEnc *self = GST_AMLH264VENC (encoder);
  GstVideoInfo *info;
 ...
 */
static GstFlowReturn
gst_amlh264venc_handle_frame (GstVideoEncoder * video_enc,
    GstVideoCodecFrame * frame)
{
  GstAmlH264VEnc *encoder = GST_AMLH264VENC (video_enc);
  GstFlowReturn ret;
  ...
  ret = gst_amlh264venc_encode_frame (encoder, frame);
...
static GstFlowReturn
gst_amlh264venc_encode_frame (GstAmlH264VEnc * encoder,
    GstVideoCodecFrame * frame)
{
  ...
  memset(&s_dma_info,0,sizeof(vl_dma_info_t));
  ...
  i4_encoder_buf_len =
      vl_video_encoder_encode(encoder->codec.handle, frame_type,NULL,info->size,
                              encoder->codec.buf, i1_format,DMA_BUFF,&s_dma_info);
...
static void
gst_amlh264venc_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstAmlH264VEnc *encoder = GST_AMLH264VENC (object);
...
static void
gst_amlh264venc_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstAmlH264VEnc *encoder = GST_AMLH264VENC (object);
...
static gboolean
amlh264venc_init (GstPlugin * amlh264venc)
{
  GST_DEBUG_CATEGORY_INIT (gst_amlh264venc_debug, "amlh264venc", 0,
      "amlogic h264 encoding element");

  return gst_element_register (amlh264venc, "amlh264venc", GST_RANK_PRIMARY,
      GST_TYPE_AMLH264VENC);
}
